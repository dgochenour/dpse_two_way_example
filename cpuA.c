...
int main(void)
{
    ...
    *DDS_StringSeq_get_reference(&dp_qos.discovery.initial_peers, 0) = 
    ...
    // and other constants in discovery_constants.h
    dr_qos.protocol.rtps_object_id = k_OBJ_ID_PARTICIANT01_DR01;
    ...
    // Now we can assert that a remote DomainParticipant (with the name held in
    // k_PARTICIANT02_NAME) will contain a DataWriter described by the 
    ...
    while (1) {
        ...
        sample->id = 1; // remember that the "id" member is keyed
        sprintf(sample->msg, "Hello from cpuA #%d\n", sample_count);
